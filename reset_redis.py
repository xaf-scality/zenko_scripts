...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description='Reset failed/pending Zenko counters in redis USE WITH CARE. Make sure "kubectl" work in your shell first.'
    )
    parser.add_argument(
        "--list", "-l", help="list available storage locations", action="store_true"
    )
    parser.add_argument(
        "--failed",
        "-f",
        help="reset failed transfers to 0 for a location",
        default=False,
    )
    parser.add_argument(
        "--pending",
        "-p",
        help="reset pending transfers to 0 for a location",
        default=False,
    )
