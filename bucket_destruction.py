    ...
    if not args.skipmarkers:
        ...
        try:
            for ver in ovs["DeleteMarkers"]:
                objs['Objects'].append(
                        {'Key': ver['Key'], 'VersionId': ver['VersionId']})
                logme("deleting marker {0} ({1}".format(
                                                ver['Key'], ver['VersionId']))
        except KeyError: pass
        except Exception as e:
            sys.stderr.write(str(e))
    ...
    for mpus in page_iterator:
        ...
        if 'Uploads' in mpus:
            ...
            for mpu in mpus['Uploads']:
                ...
                if not args.dryrun:
                    s3.abort_multipart_upload(Bucket=args.bucket, 
                            Key=mpu["Key"], UploadId=mpu['UploadId'])
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Delete versions, markers and \
stray MPUs from an S3 a bucket. You want that bucket empty for removal, don\'t \
you?')
    ...
    parser.add_argument('--prefix', default='', dest='prefix', help='delete at \
and beyond prefix')
    parser.add_argument('--noncurrent', action='store_true', help='delete only \
non-current objects (and markers)')
    parser.add_argument('--skipmarkers', action='store_true', help='skip \
deletion of delete markers')
    parser.add_argument('--skipmpus', action='store_true', help='skip clean-up \
of unfinished MPUs')
    parser.add_argument('--workers', default=WORKERS, help='number of workers \
to run (default: {0})'.format(WORKERS))
    parser.add_argument('--quiet', action='store_true', help='supress output')
    parser.add_argument('--dryrun', action='store_true', help='show objects to \
be deleted but don\'t (useless with --quiet)')
