...
kubectl_command_path = (
    "/home/centos/zenko-stack/metalk8s-1.1.0-alpha1/.shell-env/metalk8s/bin/"
)
kubectl_auth = (
    "/home/centos/zenko-stack/metalk8s/inventory/galaxy-z/artifacts/admin.conf"
)
        ...
        with open(ack_file, "r") as ft:
            ...
            try:
                if ack_t < (int(time.time()) - ack_timout):
                    send_message(
                        "ATTN: Probem with Zenko CRR Retries (repeat notice)", message
                    )
                ...
                send_message(
                    subj, "problem with failure message script: {0}".format(str(e))
                )
    else:
...
def check_retry_pods():
    ...
    retry_pods_text = (
        subprocess.Popen(
            "kubectl get pods | grep retry-failed |grep Completed | gawk '{print $1}'",
            shell=True,
            env=subenv,
    ...
    for pod in retry_pods:
        try:
            ...
            do_something(
                "Problem checking retry pod logs",
...
""".format(
                dest, failure_threshold, failure_log_ret
            )
